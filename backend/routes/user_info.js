    ...
    try {
        ...
        if (validate_email(email_address)) {
            ...
            database.sessionToEmployeeID(session_id).then((employee_id) => {
                if (employee_id) {
                    database.executeQuery(
                        `DELETE FROM tblEmail WHERE EmailAddress = '${email_address}' and EmployeeID = '${employee_id}'`
                    ).then((result) => {
        ...
        if (!phone_number.match(/^\(?\d{3}\)?[\d -]?\d{3}[\d -]?\d{4}$/)) {
            ...
            database.sessionToEmployeeID(session_id).then((employee_id) => {
                if (employee_id) {
                    ...
                    const number = digits.substring(3);
                    database.executeQuery(
                        `DELETE FROM tblPhoneNumbers WHERE AreaCode = '${area_code}' and Number = '${number}' and EmployeeID = '${employee_id}'`
                    ).then((result) => {
                        if (result.rowsAffected >= 1) {
                            res.status(200).send(
                                {
